	...
	<section>
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button size="small" type="primary" icon="el-icon-search" v-on:click="getRefractories">查询</el-button>
				</el-form-item>
				<el-form-item>
		...
		<el-table
	    :data="refractories.slice((currentPage-1)*pageSize,currentPage*pageSize)"
	    border
	    stripe
	    size="medium"
	    style="width: 100%"
	  ...
	  <el-dialog title="新增耐火材料信息" :visible.sync="newDialog">
	    ...
	    <el-form :inline="true" :model="newForm" ref="newForm" :rules="rules">  
		    ...
		    <el-form-item label="类别" prop="category">
		      <el-select v-model="newForm.category" placeholder="请选择类别">
			      <el-option label="药品类" value="药品类"></el-option>
			    </el-select>
		    </el-form-item>
		  </el-form>
	    ...
	    <el-pagination
	      ...
	      :total="this.refractories.length">
	    </el-pagination>
	  </div>
...
export default{
	...
	methods: {
		getRefractories() {
			...
			axios.get("/refractories",{ params: para }).then((response)=>{
				let res = response.data;
				this.refractories = res.result;
				this.loading = false
			}).catch((err)=>{
				console.log(err);
			});			
    ...
    checkInformation() {   //检验新建表单信息是否为空
    	...
    	for(let key in this.newForm) {
    		if(!this.newForm[key]) {
    			this.$message({ message: '请将耐火材料信息填写完整',type: 'warning',center: true });
    			this.checkOk = false;
  			  break;	
    		} else {
    			this.checkOk = true;
    ...
    submitInformation() {   //提交新建信息
        ...
        responsible: this.newForm.responsible
				...
				if (res.status==='0'){
					...
					this.$message({ message: '新建耐火材料信息成功！',type: 'success',center: true });
					this.getRefractories()			
				} else {
					this.$message({ message: '新建耐火材料信息出错，请稍后再试！',type: 'warning',center: true });
				}
			})
